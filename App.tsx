...
const App: React.FC = () => {
  ...
  const calculateActionQuality = (plan: string) => {
    ...
    score += Math.min(plan.length / 10, 20); // Length bonus
      ...
      REQUIRED_COLUMNS.forEach(col => {
        ...
        if (!match && synonyms[col]) {
          match = headers.find(h => 
            synonyms[col].some(s => h.trim().toLowerCase() === s.toLowerCase())
          );
        }
        if (!match && synonyms[col]) {
          match = headers.find(h => 
            synonyms[col].some(s => h.trim().toLowerCase().includes(s.toLowerCase()))
          );
        }
  ...
  const handleRunAiAudit = async () => {
    if (!filters.plant || !filters.issue) {
      alert("Please select both a Plant and a Chronic Issue for AI analysis.");
      return;
    }
    setAiLoading(true);
    try {
      const res = await auditActionPlan(filters.plant, filters.issue, filteredData);
      setAiAuditResult(res);
    } catch (e) {
      console.error(e);
      alert("AI Audit failed. Check API key or data complexity.");
    } finally {
      setAiLoading(false);
    }
  };
  ...
  const systemicNegligence = useMemo(() => {
    if (filteredData.length === 0) return [];
    return filteredData.filter(r => 
      (Number(r['Duration Loss']) > 50) && 
      (Number(r['Completion']) < 15) &&
      (r['Progress'] === 'Not Started' || r['Progress'] === 'Terminated')
    ).slice(0, 5);
  }, [filteredData]);
  ...
  const TABLE_COLUMNS = ["Plant Name", "Chronic Issue", "Failures Description", "Duration Loss", "Frequency", "Class", "Action Plan Vague-o-Meter", "Action Plan", "Category", "Progress", "Completion", "Start Time", "End Time", "Actions"];

  return (
    <div className="min-h-screen bg-slate-50 flex flex-col">
      <DashboardHeader />
      {(filters.plant || filters.issue || filters.class || filters.category) && (
        <button onClick={resetAllFilters} className="fixed bottom-8 right-8 z-[60] bg-blue-600 text-white px-6 py-4 rounded-2xl font-black shadow-2xl hover:bg-slate-900 transition-all flex items-center gap-3">
          <i className="fa-solid fa-filter-circle-xmark"></i> RESET FILTERS
        </button>
      )}
      <main className="flex-1 w-full px-6 py-10 lg:px-10">
        <div className="mb-10 w-full flex border-b border-slate-200">
          <div className="pb-4 text-sm font-black uppercase tracking-[0.2em] text-blue-600 border-b-2 border-blue-600">ACM Performance Intelligence</div>
        </div>
        ...
        {analysis.step === 'analyze' && analysis.data && (
          <div className="space-y-6">
            <div className="sticky top-[64px] z-[45] bg-white/95 backdrop-blur-xl -mx-6 px-6 lg:-mx-10 lg:px-10 py-6 border-b border-slate-200 shadow-sm space-y-6">
              <div className="max-w-[1600px] mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                {[
                  ...
                  <div key={filter.id} className="group">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.15em] mb-2 block">{filter.label}</label>
                    <select 
                      value={filters[filter.id as keyof typeof filters]} 
                      onChange={e => handleFilterChange(filter.id as any, e.target.value)} 
                      className={`w-full appearance-none border-2 rounded-xl px-5 py-3.5 text-[11px] font-black outline-none transition-all shadow-sm
                        ${(filters as any)[filter.id] ? 'bg-blue-600 border-blue-600 text-white' : 'bg-white border-slate-100 text-slate-700'}`}
                    >
            ...
            <div className="flex items-center justify-between border-b border-slate-200 pb-2">
              <div className="flex gap-10">
                <button onClick={() => setMainTab('ledger')} className={`pb-4 text-xs font-black uppercase tracking-[0.2em] relative ${mainTab === 'ledger' ? 'text-blue-600' : 'text-slate-400 hover:text-slate-600'}`}>
                  ACM Ledger View {mainTab === 'ledger' && <div className="absolute bottom-0 left-0 w-full h-1 bg-blue-600 rounded-t-full" />}
                </button>
                <button onClick={() => setMainTab('reporting')} className={`pb-4 text-xs font-black uppercase tracking-[0.2em] relative ${mainTab === 'reporting' ? 'text-blue-600' : 'text-slate-400 hover:text-slate-600'}`}>
                  Reporting Dashboard {mainTab === 'reporting' && <div className="absolute bottom-0 left-0 w-full h-1 bg-blue-600 rounded-t-full" />}
                </button>
                <button onClick={() => setMainTab('ai')} className={`pb-4 text-xs font-black uppercase tracking-[0.2em] relative ${mainTab === 'ai' ? 'text-blue-600' : 'text-slate-400 hover:text-slate-600'}`}>
                  AI Insight ✨ {mainTab === 'ai' && <div className="absolute bottom-0 left-0 w-full h-1 bg-blue-600 rounded-t-full" />}
                </button>
              </div>
            </div>
            {mainTab === 'ledger' ? (
              ...
              <div className="space-y-12 max-w-[1600px] mx-auto pb-20">
                ...
                <div className="bg-white p-10 rounded-[3rem] border border-slate-200 shadow-sm">
                   ...
                   <div className="relative p-10 mt-10">
                     ...
                     <div className="h-[500px] w-full relative border-l-2 border-b-2 border-slate-900">
                        <div className="absolute top-4 left-4 text-[10px] font-black text-slate-400 uppercase opacity-50">HQ Compliance</div>
                        <div className="absolute top-4 right-4 text-[10px] font-black text-red-400 uppercase opacity-50">Strategic Friction</div>
                        <div className="absolute bottom-4 left-4 text-[10px] font-black text-blue-400 uppercase opacity-50">Operational Excellence</div>
                        <div className="absolute bottom-4 right-4 text-[10px] font-black text-orange-400 uppercase opacity-50 italic">EXECUTIVE NEGLIGENCE RISK</div>
                        {dashboardStats.heatmapData.map((point, idx) => {
                          ...
                          return (
                            <div key={idx} className="absolute transition-all hover:scale-125 cursor-pointer group z-10" style={{ left: `${leftPos}%`, bottom: `${bottomPos}%`, transform: 'translate(-50%, 50%)' }}>
                               ...
                               <div className="absolute bottom-full mb-3 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 pointer-events-none transition-all scale-75 group-hover:scale-100">
                                  <div className="bg-slate-900 text-white p-4 rounded-2xl whitespace-nowrap shadow-2xl border border-white/20">
                                    <h5 className="text-[11px] font-black text-blue-400 uppercase mb-2">{point.name}</h5>
                                    <div className="space-y-1">
                                      <p className="text-[10px] font-bold flex justify-between gap-4"><span>Duration Loss:</span> <span className="text-white">{point.x} hrs</span></p>
                                      <p className="text-[10px] font-bold flex justify-between gap-4"><span>Progress:</span> <span className="text-white">{point.y}%</span></p>
                                      <p className="text-[10px] font-bold flex justify-between gap-4"><span>Frequency:</span> <span className="text-white">{point.size} events</span></p>
                                    </div>
                                  </div>
                               </div>
                            </div>
                        ...
                        <div className="absolute -left-16 top-1/2 -translate-y-1/2 -rotate-90 text-[10px] font-black text-slate-800 uppercase tracking-widest flex items-center gap-4">Execution Completion (%) <i className="fa-solid fa-arrow-right"></i></div>
                        <div className="absolute -bottom-12 left-1/2 -translate-x-1/2 text-[10px] font-black text-slate-800 uppercase tracking-widest flex items-center gap-4">Accumulated Impact (Duration Hrs) <i className="fa-solid fa-arrow-right"></i></div>
                     </div>
                   </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                  <ChartRenderer config={{ id: 'cat-dist', type: 'pie', title: 'Failure Category Contribution (%)', xAxis: 'name', yAxis: 'value' }} data={dashboardStats.categoryData} />
                  <ChartRenderer config={{ id: 'class-dist', type: 'pie', title: 'Asset Class Distribution (%)', xAxis: 'name', yAxis: 'value' }} data={dashboardStats.classData} />
                </div>
              </div>
            ) : (
              <div className="space-y-10 animate-in fade-in slide-in-from-bottom-6 duration-700 pb-20">
                {systemicNegligence.length > 0 && (
                  <div className="bg-orange-50 rounded-[3rem] p-10 border-2 border-orange-100 shadow-xl overflow-hidden relative">
                    <div className="absolute top-0 right-0 p-8 opacity-10"><i className="fa-solid fa-gavel text-8xl text-orange-600"></i></div>
                    <div className="relative z-10">
                       <h3 className="text-sm font-black text-orange-700 uppercase tracking-[0.2em] mb-6 flex items-center gap-3"><i className="fa-solid fa-radiation"></i> CTO Systemic Negligence Escalations</h3>
                       <div className="space-y-4">
                         {systemicNegligence.map((r, idx) => (
                           <div key={idx} className="bg-white p-6 rounded-2xl border border-orange-200 shadow-sm flex items-center justify-between gap-6 group hover:border-orange-400 transition-all">
                              <div className="flex items-center gap-4">
                                <div className="h-10 w-10 bg-orange-600 text-white rounded-xl flex items-center justify-center text-xs shadow-lg"><i className="fa-solid fa-exclamation-triangle"></i></div>
                                <div>
                                  <p className="text-[10px] font-black text-orange-600 uppercase tracking-widest">{r['Plant Name']} | {r['Chronic Issue']}</p>
                                  <p className="text-sm font-bold text-slate-800 italic">"Critical chronic failure unresolved: {r['Duration Loss']} hrs loss recorded with 0% progress."</p>
                                </div>
                              </div>
                              <div className="bg-orange-600 text-white px-5 py-2 rounded-full text-[9px] font-black uppercase tracking-widest shadow-lg">Negligence Risk</div>
                           </div>
                         ))}
                       </div>
                    </div>
                ...
                <div className="bg-gradient-to-br from-slate-900 via-slate-800 to-blue-900 rounded-[3rem] p-12 text-white relative overflow-hidden shadow-2xl">
                  ...
                  <div className="relative z-10 grid grid-cols-1 lg:grid-cols-3 gap-12">
                    <div className="lg:col-span-2">
                      ...
                      <h2 className="text-5xl font-black tracking-tight mb-6 italic leading-tight">ACM Action Plan Alignment</h2>
                      {aiAuditResult ? (
                        <div className="bg-white/10 backdrop-blur-md p-8 rounded-[2.5rem] border border-white/10">
                           <h3 className="text-xs font-black text-blue-400 uppercase tracking-widest mb-3 flex items-center gap-2"><i className="fa-solid fa-user-tie"></i> Local CEO Briefing Summary</h3>
                           <p className="text-lg font-medium text-slate-100 leading-relaxed italic">"{aiAuditResult.ceoBrief}"</p>
                        </div>
                      ) : (
                        <p className="text-slate-300 text-lg font-medium leading-relaxed max-w-2xl">Bridging the gap between HQ standards and plant execution. We analyze technical concrete vs. vague management speak to ensure CEO/CTO alignment on chronic issues.</p>
                      )}
                    </div>
                    <div className="flex flex-col justify-center items-center lg:items-end gap-6">
                      <button onClick={handleRunAiAudit} disabled={aiLoading} className={`group px-12 py-7 bg-blue-600 rounded-[2rem] font-black uppercase tracking-widest text-sm hover:bg-white hover:text-slate-900 transition-all shadow-2xl flex items-center gap-4 ${aiLoading ? 'opacity-50 cursor-not-allowed' : ''}`}>
                        ...
                        {aiLoading ? 'Auditing Performance...' : 'Run Strategic Audit'}
                      </button>
                      {aiAuditResult && (
                        <div className="text-center lg:text-right">
                           <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-2">Audit Health Score</span>
                ...
                {aiAuditResult && aiAuditResult.redFlags && aiAuditResult.redFlags.length > 0 && (
                  <div className="bg-red-50 rounded-[3rem] p-10 border-2 border-red-100 shadow-xl overflow-hidden relative">
                    <div className="absolute top-0 right-0 p-8 opacity-10"><i className="fa-solid fa-flag text-8xl text-red-600"></i></div>
                    <div className="relative z-10">
                       <h3 className="text-sm font-black text-red-700 uppercase tracking-[0.2em] mb-6 flex items-center gap-3"><i className="fa-solid fa-triangle-exclamation"></i> HQ Red Flags: Escalation Priorities</h3>
                       <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                         {aiAuditResult.redFlags.map((flag: string, idx: number) => (
                           <div key={idx} className="bg-white p-6 rounded-2xl border border-red-200 shadow-sm flex items-start gap-4">
                              <div className="h-8 w-8 bg-red-100 text-red-600 rounded-lg flex items-center justify-center shrink-0"><i className="fa-solid fa-circle-exclamation text-xs"></i></div>
                              <p className="text-sm font-bold text-red-900 leading-snug">{flag}</p>
                           </div>
                         ))}
                       </div>
                    </div>
                ...
                {aiAuditResult && (
                  <div className="grid grid-cols-1 gap-8 animate-in fade-in slide-in-from-bottom-8 duration-1000">
                    <div className="flex items-center justify-between px-2">
                       <h3 className="text-xs font-black uppercase tracking-[0.3em] text-slate-400">Strategic Audit Details</h3>
                       <div className="flex gap-4">
                         <div className="flex items-center gap-2"><div className="h-3 w-3 bg-red-500 rounded-full"></div><span className="text-[10px] font-black text-slate-500 uppercase">High Risk</span></div>
                         <div className="flex items-center gap-2"><div className="h-3 w-3 bg-amber-500 rounded-full"></div><span className="text-[10px] font-black text-slate-500 uppercase">Med Risk</span></div>
                         <div className="flex items-center gap-2"><div className="h-3 w-3 bg-green-500 rounded-full"></div><span className="text-[10px] font-black text-slate-500 uppercase">Low Risk</span></div>
                       </div>
                    </div>
                    <div className="space-y-10">
                      {aiAuditResult.audits.map((audit: any, i: number) => {
                        const riskColor = audit.riskLevel === 'High' ? '#ef4444' : audit.riskLevel === 'Medium' ? '#f59e0b' : '#22c55e';
                        const isOverdue = audit.ytdStatus.toLowerCase().includes('overdue');
                        const sourceAction = filteredData.find(d => d['Action Plan'] === audit.actionTitle);
                        return (
                              ...
                              <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-8 mb-10 pb-8 border-b border-slate-100">
                                <div>
                                   <div className="flex items-center gap-4 mb-3">
                                     <div className="h-10 w-10 bg-slate-900 text-white rounded-2xl flex items-center justify-center shadow-lg group-hover:bg-blue-600 transition-colors"><i className="fa-solid fa-list-check text-sm"></i></div>
                                   ...
                                   <div className="flex flex-wrap gap-3">
                                      ...
                                      <span className="text-[10px] font-black px-4 py-1.5 rounded-full uppercase border-2 flex items-center gap-2" style={{ color: riskColor, borderColor: riskColor, backgroundColor: `${riskColor}08` }}><i className="fa-solid fa-gauge-high"></i> Risk: {audit.riskLevel}</span>
                                      <span className={`text-[10px] font-black px-4 py-1.5 rounded-full uppercase border-2 flex items-center gap-2 ${isOverdue ? 'bg-red-600 text-white border-red-600' : 'bg-green-50 text-green-700 border-green-200'}`}><i className="fa-solid fa-hourglass-half"></i> {audit.ytdStatus}</span>
                                   </div>
                                </div>
                                <div className="bg-blue-600 text-white px-8 py-5 rounded-[2rem] text-center shrink-0 shadow-xl shadow-blue-100 group-hover:scale-105 transition-transform"><span className="text-[9px] font-black uppercase tracking-[0.2em] block mb-2 opacity-80">CTO Recommendation</span><span className="text-sm font-black uppercase">{audit.recommendation}</span></div>
                              </div>
                              <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
                                 <div className="lg:col-span-1">
                                    <div className="flex items-center gap-3 mb-4"><div className="h-8 w-8 bg-amber-50 text-amber-600 rounded-lg flex items-center justify-center text-xs"><i className="fa-solid fa-comment-dots"></i></div><label className="text-[11px] font-black text-slate-400 uppercase tracking-widest">CEO Alignment Point</label></div>
                                    <div className="bg-amber-50/50 p-6 rounded-3xl border border-amber-100 relative mb-6"><div className="absolute top-0 right-0 p-4 opacity-10"><i className="fa-solid fa-quote-right"></i></div><p className="text-sm text-amber-900 font-bold leading-relaxed italic">"{audit.ceoTalkingPoint}"</p></div>
                                    {sourceAction && (
                                      <div className="bg-slate-50 p-6 rounded-3xl border border-slate-100"><div className="flex items-center gap-3 mb-3"><i className="fa-solid fa-bug text-slate-400 text-xs"></i><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Failure Context</label></div><p className="text-sm font-bold text-slate-800 leading-relaxed italic break-words">{sourceAction['Failures Description']}</p></div>
                                    )}
                                 </div>
                                 <div className="lg:col-span-1 space-y-6">
                                    <div><label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] block mb-3">Remote Trackability</label><p className="text-sm text-slate-700 font-bold leading-relaxed flex items-start gap-3"><i className="fa-solid fa-satellite-dish text-blue-500 mt-1"></i>{audit.trackability}</p></div>
                                    <div><label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] block mb-3">Quality Rating</label><p className="text-sm font-black text-slate-900 uppercase tracking-tighter italic">{audit.qualityRating}</p></div>
                                 </div>
                                 <div className="lg:col-span-1"><label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] block mb-3">Audit Justification</label><p className="text-sm text-slate-600 font-medium leading-relaxed italic bg-slate-50 p-6 rounded-3xl border border-slate-100">"{audit.justification}"</p></div>
                              </div>
                            </div>
                          </div>
                        );
